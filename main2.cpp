...
void calculateVorticity() {
    std::cout << "calculateVorticity start" << std::endl;
    for (size_t i = 0; i < PrimalVertices.size(); i++) {
        vector <Vec3> &vertices = DualFaces[i];
        ...
        for (size_t j = 0; j < vertices.size(); j++) {
             ...
             int tri = findTriangle(vertices[j].x, vertices[j].y);
             if (tri == -1) {
                 tri = findClosestTriangle(vertices[j].x, vertices[j].y);
             }
    ...
    std::cout << "totalW before " << totalW << std::endl;
    ...
    std::cout << "Total w after: " << totalW << std::endl;
    std::cout << "All w:" << fullW << std::endl;
    std::cout << "calculateVorticity end" << std::endl;
...
void moveParticles() {
    for (size_t i = 0; i < Particles.size(); i++) {
        Vec3 &position = Particles[i];
        Triangle t = PrimalTriangles[position.z];

        //std::cout << "t: " << t.index << " " << t.x << " " << t.y << std::endl;
        Vec3 v = interpolateVelocity(position.x, position.y);
        //std::cout << "Vel: " << v.x << " " << v.y << std::endl;
        double scale = 50.0;
        
        
        if (findTriangle(position.x + v.x/scale, position.y + v.y/scale) == -1) {
            scale = 100.0;
        ...
        */
        ...
        int tri = findTriangle(position.x + v.x/scale, position.y + v.y/scale);
        if (tri == -1) {
            /* could potential just restart particles at origin..
            Particles[i].x = 0;
            Particles[i].y = 0;
            tri = findTriangle(position.x + v.x/scale, position.y + v.y/scale);
            */
            continue;
        }
        Particles[i].z = tri;
...
void drawObject() 
{
    ...
    if (currentSelection != -1) { // draw selected triangle (if any)
        ...
        if (currentDualFace != -1) {
            const vector <Vec3> face = DualFaces[currentDualFace];
            glColor3d(0,1,0);
            glBegin(GL_POLYGON);
            for (size_t j = 0; j < face.size(); j++) {
                Vec3 vertex = face[j];
...
void setupDeltaInterior() {
    std::cout << "setupDeltaInterior start" << std::endl;
    if (gDebug) {
    ...
    std::cout << "setupDeltaInterior end " << std::endl;
}

/* Updates flux values on each edge, given vorticities */
void calculateFlux() {

    std::cout << "calculateFlux start" << std::endl;
    ...
    */
    ...
    std::cout << "sumFluxes before " << sumFluxes << std::endl;
    ...
    std::cout << "sumFluxes after " << sumFluxes << std::endl;

    std::cout << "calculateFlux end" << std::endl;
}
...
void updateEverything() {
    ...
    std::cout << "Calculate Frame: " << ++gFrame << std::endl;
    calculateVorticity();
    calculateFlux();
    calculateVelocity();
    moveParticles();
...
void sortVertices2(vector <Vec3> &dualFace) { 
    double min_y = 1.0;
    int minVertex;
    ...
    Vec3 minimumVertex = dualFace[minVertex];

    for (size_t i = 0; i < dualFace.size(); i++) {
        const Vec3 &v = dualFace[i];
...
void setupEigenH0() {
    ...
    for (size_t i = 0; i < PrimalVertices.size(); i++) {
        ...
        vector <Vec3> &dualFace = DualFaces[i];
        if (dualFace.size() == 0) {
...
void setupInitialFlux() {
    //PrimalVertices[23].vorticity = 1; // was 21
    //PrimalVertices[25].vorticity = -1;

    //PrimalVertices[16].vorticity = 1; // was 21
    //PrimalVertices[7].vorticity = -1;

    //PrimalVertices[207].vorticity = 1; // was 70
    //PrimalVertices[213].vorticity = 1; // was 270
    //PrimalVertices[210].vorticity = 0.1;

    /*
    PrimalVertices[157].vorticity = 1.0/9.0; // was 70
    PrimalVertices[158].vorticity = 1.0/9.0; // was 70
    PrimalVertices[159].vorticity = 1.0/9.0; // was 70
    PrimalVertices[176].vorticity = 1.0/9.0; // was 70
    PrimalVertices[177].vorticity = 10.0/9.0; // was 70
    PrimalVertices[178].vorticity = 1.0/9.0; // was 70
    PrimalVertices[195].vorticity = 1.0/9.0; // was 70
    PrimalVertices[196].vorticity = 1.0/9.0; // was 70
    PrimalVertices[197].vorticity = 1.0/9.0; // was 70

    PrimalVertices[163].vorticity = 1.0/9.0; // was 70
    PrimalVertices[164].vorticity = 1.0/9.0; // was 70
    PrimalVertices[165].vorticity = 1.0/9.0; // was 70
    PrimalVertices[182].vorticity = 1.0/9.0; // was 70
    PrimalVertices[183].vorticity = 10.0/9.0; // was 70
    PrimalVertices[184].vorticity = 1.0/9.0; // was 70
    PrimalVertices[201].vorticity = 1.0/9.0; // was 70
    PrimalVertices[202].vorticity = 1.0/9.0; // was 70
    PrimalVertices[203].vorticity = 1.0/9.0; // was 70
    */
    ...
    std::cout << "gSumVorticity " << gSumVorticity << std::endl;
    ...
    //PrimalVertices[183].vorticity = 1; // was 270
    //PrimalVertices[199].vorticity = 1.5;

    //PrimalVertices[860].vorticity = 1; // was 70
    //PrimalVertices[880].vorticity = 1; // was 270
    
    /*
    for (size_t i = 0; i < PrimalVertices.size(); i++) {
        PrimalVertices[i].vorticity = 1;
    }
    */
...
void solveLaplacian() {
    ...
    std::cout << "solveLaplacian start" << std::endl;
    ...
    std::cout << "solveLaplacian end" << std::endl;
}
...
int main(int argc, char* argv[])
{
    ...
    for (size_t i = 0; i < PrimalVertices.size(); i++) {
        ...
        if (vertex.boundary == 1) { 
            ...
            Vec3 actualVertex;
            actualVertex.x = vertex.x;
            actualVertex.y = vertex.y;
        ...
        for (size_t j = 0; j < PrimalTriangles.size(); j++) {
            ...
            if (triangle.v1 == vertexNum || triangle.v2 == vertexNum || triangle.v3 == vertexNum) {
                ...
                Vec3 dualVertex;
                dualVertex.x = triangle.x;
                dualVertex.y = triangle.y;
                //dualVertex.z = j; // store the index here
    ...
    std::cout << "setupEigen start" << std::endl;
    setupEigenD0();
    setupEigenD1();

    //Eigen::SparseMatrix<double> mat(10, 10);
    //std::cout << "d1: " << eigenD1.cols() << " " << eigenD1.rows() << std::endl;;
    //std::cout << "d0: " << eigenD0.cols() << " " << eigenD0.rows() << std::endl;;
    ...
    setupEigenH1();
    setupEigenH2();
    std::cout << "setupEigen end" << std::endl;
    ...
    Eigen::SparseMatrix<double> matTranspose(10, 10);
    matTranspose = mat.transpose();
    std::cout << matTranspose << std::endl;

    Eigen::VectorXd b1(10), x;
    for (int i=0; i<10; i++) {
        b1[i] = i;
    }  
    ...
    for (size_t i = 0; i < PrimalVertices.size(); i++) {
        Vertex v = PrimalVertices[i];
        std::cout << v.index << " " << v.x << " " << v.y << " " << v.boundary << std::endl;
    }
    Vertex AA;
    std::cout << AA.index << " " << AA.x << " " << AA.y << std::endl;
    AA.index = 5;
    AA.x = 9;
    AA.y = 10;
    std::cout << AA.index << " " << AA.x << " " << AA.y << std::endl;
